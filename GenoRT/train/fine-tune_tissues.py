...
w82_tsses = pd.read_table(
   rf"/Data5/pfGao/xtwang/TSS/tss/data_process/W82.{tissue}.Unidirection_result.txt",
    sep="\t",
)
...
w82_tfeature = GenomicSignalFeatures(
  [
    rf"data_process/filter_bw/W82.{tissue}.plus.filter.bw",
    rf"data_process/filter_bw/W82.{tissue}.minus.filter.bw",
],
    [
        f"{tissue}_plus",
        f"{tissue}_minus",
...
# w82_tsses.to_csv('/Data5/pfGao/xtwang/TSS/tss/data_process/W82.Unidirection_result.txt',sep='\t',index=False)
...
for epoch in range(0,5):
    ...
    pre_train = torch.load(glob.glob(rf"/Data5/pfGao/xtwang/TSS/tss/new_model/W82_for_draw/no*pt",)[epoch % 5]).state_dict()
# 加载权重到模型
    missing_keys, unexpected_keys = net.load_state_dict(pre_train, strict=False)
    print(net)
    for name, param in net.named_parameters():
        if "reg_final" in name or f'.down' in name or f"decoder.u" in name or 'bias' in name:
    ...
    for iter in range(10):
        ...
        for j in range(train_seqs.shape[0] // batchsize):
            ...
            if i % 200 == 0:
                ...
                if testcor > bestcor:
                    try:
                        os.remove(f'new_model/W82_tissues/w82_fine-tune_predict_pi_w82_merge_stage_3_for_{tissue}_{epoch}_avgcor_{"{:.7f}".format(bestcor)}.best.pt')
                    except FileNotFoundError:
                        pass
                    torch.save(net,f'new_model/W82_tissues/w82_fine-tune_predict_pi_w82_merge_stage_3_for_{tissue}_{epoch}_avgcor_{"{:.7f}".format(testcor)}.best.pt')
                    bestcor = testcor
                    counter = 0
                else:
                    counter += 1
